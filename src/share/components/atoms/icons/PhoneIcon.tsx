export const MODE_PHONE_ICON = {
    DEFAULT: 'DEFAULT',
    WHITE: 'WHITE',
} as const;
export type ModePhoneIconType =
    (typeof MODE_PHONE_ICON)[keyof typeof MODE_PHONE_ICON];

const PhoneIcon = ({
    mode = MODE_PHONE_ICON.DEFAULT,
    size = 24,
}: {
    mode?: ModePhoneIconType;
    size?: number;
}) => {
    const getFillColor = () => {
        switch (mode) {
            case MODE_PHONE_ICON.DEFAULT:
                return '#1E1E1E';
            case MODE_PHONE_ICON.WHITE:
                return '#FFFFFF';
        }
    };
    return (
