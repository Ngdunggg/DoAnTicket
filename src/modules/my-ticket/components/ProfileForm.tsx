...
import CameraIcon, { MODE_CAMERA } from "@share/components/atoms/icons/CameraIcon";
...
const ProfileForm = () => {
    ...
    const renderAvatarSection = () => (
        <div className="flex items-center justify-center flex-col gap-10">
            <DivClick className="w-fit h-fit relative cursor-pointer" onClick={handleAvatarClick}>
                <img
                    src={formData.avatar}
                    alt="avatar"
            ...
            >
                Cung cấp thông tin chính xác sẽ hỗ trợ bạn trong quá trình mua vé, hoặc khi cần xác
                thực vé
            </Text>
        </div>
    );
            ...
            <div className="flex gap-14">
                {GENDER_OPTIONS.map(({ value, label }) => (
                    <div key={value} className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor={`gender-${value}`} className="text-white cursor-pointer">
                            {label}
                        </label>
                    </div>
                ))}
    ...
    return (
        <div className="flex flex-col flex-1 py-14 px-6">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col items-center flex-1 gap-6">
