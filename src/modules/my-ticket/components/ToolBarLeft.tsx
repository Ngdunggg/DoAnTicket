...
import { MODE_COLOR_TEXT, MODE_SIZE, Text, MODE_WEIGHT } from "@share/components/atoms/Text";
    ...
    return (
        <div className="flex flex-col px-6 gap-10">
            <div className="flex items-center gap-1">
                ...
                />
                <Text
                    modeColor={!isProfilePage ? MODE_COLOR_TEXT.YELLOW : MODE_COLOR_TEXT.WHITE}
                    modeSize={MODE_SIZE[16]}
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-2">
                    <Text modeColor={MODE_COLOR_TEXT.WHITE} modeSize={MODE_SIZE[14]}>
                        Tài khoản của
                    </Text>
                    <Text
            ...
            <div className="flex flex-col px-14 gap-5">
                <DivClick
                    className={`flex items-center gap-2 cursor-pointer transition-colors duration-200 ${
                        isProfilePage ? "text-text-yellow" : "text-white hover:text-text-yellow"
                    }`}
                    onClick={() => navigate(Path.PathMyProfile)}
                >
                    <UserIcon mode={isProfilePage ? MODE_USER.YELLOW : MODE_USER.DEFAULT} />
                    <Text
                        modeColor={isProfilePage ? MODE_COLOR_TEXT.YELLOW : MODE_COLOR_TEXT.WHITE}
                    >
                        Thông tin tài khoản
                ...
                <DivClick
                    ...
                    onClick={() => navigate(Path.PathMyTicket)}
                >
                    <TicketIcon mode={!isProfilePage ? MODE_TICKET.YELLOW : MODE_TICKET.WHITE} />
                    <Text
                        modeColor={!isProfilePage ? MODE_COLOR_TEXT.YELLOW : MODE_COLOR_TEXT.WHITE}
                    >
                        Vé của tôi
                ...
                <DivClick className="flex items-center gap-2" onClick={() => {}}>
                    <EventIcon />
                    <Text modeColor={MODE_COLOR_TEXT.WHITE}>Sự kiện của tôi</Text>
                </DivClick>
            </div>
        </div>
    );
